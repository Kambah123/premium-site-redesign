...
// I am replacing the hero manually.

const startToken = '{/* ── Main Content ──────────────────────────────── */}';
const newHeroCode = `{/* ── Hero & Cart Section ─────────────────────── */}
...
// let's insert it before the Main Content block which is a safe anchor.
code = code.replace(startToken, newHeroCode + startToken);

// But we also need to remove the OLD hero and compliance banner
// Instead of complex string manipulation, I'll just use regex from the breadcrumbs to the Main Content
code = code.replace(/\{(\/\* ── Hero[ \S]*?\*\/)\}[\s\S]*?(?=\{\/\* ── Main Content)/, '');
code = code.replace(/<div className="pt-28 pb-0 bg-pearl-shimmer\/50 border-b border-navy-900\/5">[\s\S]*?(?=\{\/\* ── Hero & Cart)/, '');
