...
// The hero rewrite failed in my previous script because the string didn't match.
// Let's rewrite it explicitly now that we know what's there.

const heroStart = code.indexOf('<section className="bg-pearl-shimmer/50 pb-20 pt-12 relative overflow-hidden border-b border-navy-900/5">');
const heroEnd = code.indexOf('{/* ── Compliance Banner (Prominent) ──────────────── */}');

if (heroStart === -1) {
    // maybe it still has the old hero?
    const oldHeroStart = code.indexOf('<section className="bg-pearl-shimmer/50 pb-20 pt-12 relative overflow-hidden">');
    if (oldHeroStart !== -1) {
      ...
      `;
        code = code.substring(0, oldHeroStart) + newHero + code.substring(heroEnd);
